...
def is_docker_running() -> bool:
    ...
    try:
        ...
        if "Cannot connect to the Docker daemon" in process.stderr or \
           "Is the docker daemon running?" in process.stderr or \
           process.returncode != 0: # Consider any non-zero return code as a potential issue for simplicity here
        ...
        console.print("⏰ Timeout while trying to connect to Docker daemon.", style="yellow")
        return False
    except Exception as e:
        # Catch any other unexpected errors during the check
        console.print(f"⚠️ Unexpected error while checking Docker status: {e}", style="yellow")
        return False
...
def does_screen_session_exist(session_name: str) -> bool:
    ...
    try:
        ...
        if process.returncode > 1 and "No Sockets found" not in process.stdout and "No Sockets found" not in process.stderr:
            console.print(f"[dim]Error running 'screen -ls'. RC: {process.returncode}, Stderr: {process.stderr.strip()}[/dim]", style="yellow")
            return False
        ...
        console.print("❌ 'screen' command not found. Is screen installed?", style="red")
        ...
        console.print(f"⚠️ Unexpected error while checking screen session status: {e}", style="yellow")
        return False
...
def list_snapshotter_screen_sessions() -> list[dict[str, str]]:
    ...
    try:
        ...
        console.print(f"⚠️ Unexpected error while listing screen sessions: {e}", style="yellow")
        return sessions
