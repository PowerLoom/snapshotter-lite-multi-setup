...
def get_docker_container_status_for_instance(
    ...
    if not chain_name_upper or not market_name_upper:
        ...
        console.print("[dim]Skipping Docker search: chain name or market name not provided for filtering.[/dim]", style="yellow")
        return containers
    ...
    try:
        ...
        for line in output_lines:
            ...
            if len(raw_parts) >= 2: # Minimum: name and ID. Status can be empty or multi-word.
                name = raw_parts[0]
                ...
                console.print(f"[dim yellow]⚠️ Skipping Docker ps output line: not enough parts after space split: '{line}'[/dim yellow]")
                continue
            
            name_upper = name.upper() # For case-insensitive comparison of constant parts
            ...
            if slot_id: # Slot ID is crucial for the node container
                ...
                if (
                    f"SNAPSHOTTER-LITE-V2" in name_upper and
                    slot_id in name and # slot_id is numeric, check in original case name
                    chain_name_upper in name_upper and
                    market_name_upper in name_upper
                ):
                    is_node_match = True
            ...
            if (
                f"SNAPSHOTTER-LITE-LOCAL-COLLECTOR" in name_upper and
                chain_name_upper in name_upper and
                market_name_upper in name_upper
            ):
        ...
        if not containers:
            console.print(f"[dim]No Docker containers found matching patterns for slot {slot_id}, chain {chain_name_upper}, market {market_name_upper}[/dim]", style="yellow")
        ...
        console.print(f"[dim]Error running 'docker ps': {e.stderr.strip() if e.stderr else 'No stderr'}[/dim]", style="yellow")
        return []
